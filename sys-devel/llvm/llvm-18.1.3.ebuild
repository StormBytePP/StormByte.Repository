...
multilib_src_configure() {
	...
	# If clang-common[polly] is enabled, somehow gentoo-runtimes.cfg is not respected
	# so to avoid compile failures when polly is used we need to manually load
	# LLVMPlugin.so if we are using clang
	tc-is-clang && has_version ~sys-devel/clang-common-${PV}[polly] && append-flags -fplugin=LLVMPolly.so
