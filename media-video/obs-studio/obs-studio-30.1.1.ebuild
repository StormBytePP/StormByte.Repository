...
prepare_cef() {
	OBS_BUILD_DIR="${BUILD_DIR}"
	OBS_CMAKE_USE_DIR="${CMAKE_USE_DIR}"
	cd "${WORKDIR}/${CEF_DIR}"
	rm -Rf build
	BUILD_DIR="${WORKDIR}/${CEF_DIR}/build"
	CMAKE_USE_DIR="${WORKDIR}/${CEF_DIR}"
	if tc-is-clang; then
		eapply "${FILESDIR}/clang-update-deprecated-builtins.patch"
	fi
	cmake_src_prepare
	cd "${S}"
	BUILD_DIR="${OBS_BUILD_DIR}"
	CMAKE_USE_DIR="${OBS_CMAKE_USE_DIR}"
}
...
src_prepare() {
	...
	cmake_src_prepare
	use browser && prepare_cef && compile_cef
}
